...
<div class="heading-buttons">
	...
	<div class="buttons pull-right">
		<a href="${pageContext.request.contextPath}/main/shopAction!addUI.action" class="btn btn-primary btn-icon glyphicons circle_plus"><i></i> 添     加  </a>
				...
				<table class="table table-bordered table-condensed table-striped table-primary table-vertical-center checkboxs js-table-sortable">
					...
					<tbody>
						<s:iterator value="goods" status="stats">
							...
							<td class="center">&euro;${price}</td>
						    
							<td class="center form-inline small">
								<input type="text" style="width: 30px;" value="${stock}" readonly="readonly"/>
							</td>
							<td class="center important">图片数<s:property value="img.size"/><span class="glyphicons btn-action single picture" style="margin-right: 0;"><i></i></span><s:iterator value="img" var="image" status="status"> <a href="${pageContext.request.contextPath}/${image}" rel="lightbox" title="图片 <s:property value='#status.index'/>" > p<s:property value="#status.index"/></a></s:iterator></td>
							<td><strong><s:if test="status==1">已经上架</s:if><s:elseif test="status==0">正在审核</s:elseif><s:else>已经下架</s:else> </strong></td>
							<td class="center">
								<a href="${pageContext.request.contextPath}/main/shopAction!editUI.action?id=${id}" class="btn-action glyphicons pencil btn-success"><i></i></a>
								<a href="${pageContext.request.contextPath}/main/shopAction!delete.action?id=${id}"  onclick="return window.confirm('您确定要删除当前选中商品吗？')" class="btn-action glyphicons remove_2 btn-danger"><i></i></a>
